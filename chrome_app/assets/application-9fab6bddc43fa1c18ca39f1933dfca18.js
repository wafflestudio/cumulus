...
//var GOOGLE_DRIVE_CLIENT_SECRET = 'I1twwV3hhnnc6SKGy5if3tct'; // localhost:3301 TaekminKim
//var GOOGLE_DRIVE_CLIENT_ID = '149164205354.apps.googleusercontent.com'; // localhost:3301 BeomkiLee
var GOOGLE_DRIVE_CLIENT_ID = '270869421562-7b3osl0upgas7u7th8q8slinsf4m0n4t.apps.googleusercontent.com'; // chrome-extension://hjiamomkaeiaackbgpoopfefnojhjpfj/
var GOOGLE_DRIVE_SCOPES = 'https://www.googleapis.com/auth/drive';
//var DROPBOX_DRIVE_KEY = '4s7mj1rlj7xirhj';
var DROPBOX_DRIVE_KEY = 'kj1ty95hy096yqt'; // chrome-extension://hjiamomkaeiaackbgpoopfefnojhjpfj/
;
/*!
...
(function() {
  ...
  window.intersection = function(a, b) {
...
(function() {
  this.Explorer = (function() {
    function Explorer() {
      this.tabs = [new Tab(fileSystem.rootIds)];
    }

    Explorer.prototype.currentTab = function() {
      currentTab;
      var currentTab, tab, _i, _len, _ref;
      _ref = this.tabs;
  ...
  this.Tab = (function() {
    ...
    Tab.prototype.body = function() {
      return $('#explorer > tbody');
    };

    Tab.prototype.isFocused = function() {
      return true;
    };

    Tab.prototype.isRoot = function() {
      return this.parentPaths.empty();
    };

    Tab.prototype.open = function(fileIds) {
      var path, _i, _len, _ref;
      console.log("enter " + fileIds);
      this.parentPaths = this.paths;
      if (fileIds instanceof Array) {
        this.paths = fileIds;
      } else {
        this.paths = [fileIds];
      }
      if (!intersection(this.paths, fileSystem.rootIds).empty()) {
    ...
    Tab.prototype.copy = function(from, to) {};

    Tab.prototype.move = function(from, to) {};

    Tab.prototype.remove = function(fileId) {};

    Tab.prototype.renderDirectory = function(files) {
      var $td, $title, $tr, body, file, fileLink, _i, _len, _this;
      body = this.body();
      body.html('');
      if (!this.isRoot()) {
        $tr = $('<tr>').attr('file-id', this.parentPaths[0]);
        $title = $('<td>').html($('<a>').attr('href', '#' + this.parentPaths[0]).text(".."));
        $title.appendTo($tr);
        $('<td>').html("").appendTo($tr);
        $tr.appendTo(body);
        _this = this;
      ...
      for (_i = 0, _len = files.length; _i < _len; _i++) {
        ...
        if (file.downloadUrl) {
        ...
        } else if (!file.isDirectory() && file.id.indexOf('/') === 0) {
          ...
          $('<td class="downloadurl">').appendTo($tr);
        }
        $tr.appendTo(body);
        if (file.isDirectory()) {
          _this = this;
      ...
      $('.downloadurl-dropbox a').click(function(e) {
        var id;
        id = $(this).parent().parent().attr('file-id');
    ...
    Tab.prototype.render = function() {
      var file, files, filesInDirectory, _i, _j, _len, _len1;
      if (this.paths.empty()) {
        this.paths = fileSystem.rootIds;
      }
      filesInDirectory = [];
      files = fileSystem.files;
      if (this.parentPaths.empty()) {
        ...
        for (_j = 0, _len1 = files.length; _j < _len1; _j++) {
          file = files[_j];
          console.log(intersection(this.paths, file.parentIds));
          if (!intersection(this.paths, file.parentIds).empty()) {
            filesInDirectory.push(file);
          }
        }
      }
      return this.renderDirectory(filesInDirectory);
    };

    Tab.prototype.renderPhoto = function() {
      var file, files, filesInDirectory, _i, _len;
      if (this.path === null) {
        this.path = fileSystem.rootId;
      }
      filesInDirectory = [];
      files = fileSystem.files;
      for (_i = 0, _len = files.length; _i < _len; _i++) {
        file = files[_i];
        if (file.isPhoto() && !intersection(this.paths, file.parentIds).empty()) {
          filesInDirectory.push(file);
        }
      }
      return this.renderDirectory(filesInDirectory);
...
(function() {
  this.FileSystem = (function() {
    ...
    FileSystem.prototype.push = function(file) {
      if (!this.filesIndex[file.id]) {
        this.files.push(file);
        return this.filesIndex[file.id] = this.files.length - 1;
      }
    };

    FileSystem.prototype.addRoot = function(rootId) {
      this.rootIds.push(rootId);
      return this.rootIds.unique();
    };

    FileSystem.prototype.getFile = function(fileId) {
      console.log("fileId: " + fileId);
      return this.files[this.filesIndex[fileId]];
    };
  ...
  this.File = (function() {
    function File(id, parentIds, isRoot, mimeType, title, downloadUrl, previewUrl) {
      this.id = id;
    ...
    File.prototype.isDirectory = function() {
      var _ref;
      return (_ref = this.mimeType) === 'application/vnd.google-apps.folder' || _ref === "inode/directory";
    };

    File.prototype.isAudio = function() {
      var _ref;
      return (_ref = this.mimeType) === 'audio/mpeg';
    };

    File.prototype.isPhoto = function() {
      var _ref;
      return (_ref = this.mimeType) === 'image/jpeg' || _ref === 'image/pjpeg' || _ref === 'image/png';
    };

    File.prototype.isArchive = function() {
      var _ref;
      return (_ref = this.mimeType) === 'application/xzip';
    };

    File.prototype.isDocument = function() {
      var _ref;
      return (_ref = this.mimeType) === 'application/vnd.google-apps.document';
    };

    File.prototype.isSpreadsheet = function() {
      var _ref;
      return (_ref = this.mimeType) === 'application/vnd.ms-excel' || _ref === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' || _ref === 'application/vnd.google-apps.spreadsheet';
    };

    File.prototype.isForm = function() {
      var _ref;
      return (_ref = this.mimeType) === 'application/vnd.google-apps.form';
    };

    File.prototype.typeIcon = function() {
      var addType, glyphicon;
      glyphicon = '<span class="glyphicon"></span>';
      addType = function(className) {
        return $(glyphicon).addClass(className);
      };
      if (this.isDirectory()) {
        return addType('glyphicon-folder-close');
      } else if (this.isAudio()) {
        return addType('glyphicon-music');
      } else if (this.isArchive()) {
        return addType('glyphicon-compressed');
      } else if (this.isDocument()) {
        return addType('glyphicon-file');
      } else if (this.isSpreadsheet()) {
        return addType('glyphicon-tasks');
      } else if (this.isForm()) {
        return addType('glyphicon-list-alt');
      } else {
        return addType('glyphicon-question-sign');
      }
    };
...
(function() {
  ...
  this.GoogleDriveClient = (function(_super) {
    __extends(GoogleDriveClient, _super);

    function GoogleDriveClient(clientId, scopes) {
      this.checkAuth = __bind(this.checkAuth, this);
      this.handleAuthResult = __bind(this.handleAuthResult, this);
      this.clientId = clientId;
      this.scopes = scopes;
      gapi.client.setApiKey('AIzaSyAH5BivYV9VBWBrIuyXUfaMt2rY0c5bEpg');
      console.log("GoogleDrive(" + this.clientId + ", " + this.scopes + ")");
    }

    GoogleDriveClient.prototype.listFiles = function(rootFolderId) {
      var request;
      ...
      return request.execute(function(data) {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          ...
          file = new File(item.id, item.parents, item.parents.length === 0 || __indexOf.call(item.parents, rootFolderId) >= 0, item.mimeType, item.title, item.webContentLink, item.alternateLink);
    ...
    GoogleDriveClient.prototype.getUserInfo = function() {
      var request,
        _this = this;
      request = gapi.client.request({
        path: "/drive/v2/about",
      ...
      return request.execute(function(data) {
        ...
        return _this.listFiles(data.rootFolderId);
      });
    };

    GoogleDriveClient.prototype.handleAuthResult = function(authResult) {
      var authButton,
        _this = this;
      ...
      if (!(authResult && !authResult.error)) {
        $('#form-authorize').show();
        return authButton.onclick = function() {
    ...
    GoogleDriveClient.prototype.checkAuth = function() {
      console.log(this.clientId, this.scopes);
      return gapi.auth.authorize({
  ...
  this.DropboxClient = (function(_super) {
    __extends(DropboxClient, _super);

    function DropboxClient(key) {
      this.getDownloadLink = __bind(this.getDownloadLink, this);
      this.client = new Dropbox.Client({
        key: key
      });
    }

    DropboxClient.prototype.authorize = function() {
      var _this = this;
      return this.client.authenticate(function(error, data) {
        ...
        fileSystem.addRoot("/");
        _this.listFiles('/');
        return _this.client.getAccountInfo(function(error, userInfo) {
          var $li;
          if (error) {
          ...
          return $('#user-info').append($li);
        });
      });
    };

    DropboxClient.prototype.getDownloadLink = function(path) {
      var _this = this;
      return this.client.makeUrl(path, {
        download: true
      }, function(error, data) {
        if (error) {
          return console.log(error);
        ...
        return alert("api.js.coffee#84 " + data.url);
      });
    };

    DropboxClient.prototype.listFiles = function(path) {
      var _this = this;
      return this.client.readdir(path, function(error, entries, dir_stat, entry_stats) {
        var entry, file, _i, _len;
        if (error) {
          console.log(error);
          if (error.status === 429) {
            _this.client.authenticate(function(error, data) {
              console.log(data);
              return _this.listFiles(path);
            });
          } else {
            return error;
          }
        ...
        for (_i = 0, _len = entry_stats.length; _i < _len; _i++) {
          entry = entry_stats[_i];
          file = new File(entry.path, path, path === "/", entry.mimeType, entry.name, null);
          fileSystem.push(file);
        }
        return explorer.render();
      });
    ...
    DropboxClient.prototype.isAuthorized = function() {
...
(function() {
  ...
  window.handleClientLoad = function() {
    ...
    return googleDriveClient.checkAuth();
  };
  ...
  $(function() {
    return $('#photo-album').click(function() {
      if ($('#photo-album').html() === "Photo Album") {
        explorer.currentTab().renderPhoto();
        return $('#photo-album').html("Go Back");
      } else {
        explorer.currentTab().render();
        return $('#photo-album').html("Photo Album");
      }
    });
  });
